...
# .env 사용을 중단하고 Streamlit secrets를 사용합니다.
...
def generate_ai_insights(df, stats):
    ...
    try:
        # API 키 확인 (Streamlit secrets → 환경변수 순 백업)
        api_key = st.secrets.get('GEMINI_API_KEY') or os.getenv('GEMINI_API_KEY')
        st.write(f"🔍 API 키 확인: {'설정됨' if api_key else '설정되지 않음'}")
        if not api_key:
            return "⚠️ Gemini API 키가 설정되지 않았습니다. `.streamlit/secrets.toml` 또는 배포 환경의 시크릿에 `GEMINI_API_KEY`를 설정해주세요."
...
if df is not None:
    ...
    # API 키 설정 안내 (Streamlit secrets 우선 확인)
    if not (st.secrets.get('GEMINI_API_KEY') or os.getenv('GEMINI_API_KEY')):
        ...
        2. 로컬 실행: `.streamlit/secrets.toml` 파일을 생성하고 아래처럼 설정하세요
           
           GEMINI_API_KEY = "your_api_key_here"
           
        3. Streamlit Community Cloud 또는 서버 배포 시, 해당 플랫폼의 시크릿 관리에 `GEMINI_API_KEY`를 등록하세요
        
        자세한 설정 방법은 `README_AI_DASHBOARD.md`를 참고하세요.
        """)
